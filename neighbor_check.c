...
/*   Updated: 2026/09/01 14:48:42 by mramaros         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	my_turn_fifo(t_coders *coder)
{
	...
	while (all_coders)
	{
		if (all_coders->id != coder->id && my_neighbor(coder, all_coders))
			if (all_coders->id != coder->id && all_coders->ticket > 0
				&& all_coders->ticket < coder->ticket)
				return (0);
...
int	my_turn_edf(t_coders *coder)
{
	...
	my_deadline = coder->last_time_for_compile
		+ coder->data->time_to_burnout;

	while (tmp)
	{
		if (tmp->id != coder->id && my_neighbor(coder, tmp))
		{
			if (tmp->ticket > 0)
			{
				...
				if (other_deadline > my_deadline)
					return 0;
				else if (other_deadline == my_deadline)
					return 0;
			}
		}
		tmp = tmp->next;
